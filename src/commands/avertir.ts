...
const warnCommand: Command = {
    ...
    async run(bot: ExtendedClient, interaction: ChatInputCommandInteraction) {
        ...
        // Vérifier si c'est l'utilisateur lui-même qui essaye de s'appliquer l'avertissement
        ...
        try {
            ...
            await interaction.reply(
                errorMessage(
                    bot,
                    "L’utilisateur que tu veux avertir n’est pas présent sur ce serveur.",
        ...
        // Vérifier si l'utilisateur peut reçevoir un avertissement
        if (!member?.moderatable) {
            await interaction.reply(
                errorMessage(
                    bot,
                    "Cet utilisateur ne peut pas recevoir d’avertissement.",
                    "Permission refusée",
                    "0x203",
                    false
                )
        ...
        // Vérifier si l'utilisateur qui tente d'avertir à un rôle plus élevé
        ...
        if (executorHighestRole.position <= targetHighestRole.position) {
            await interaction.reply(
                errorMessage(
                    bot,
                    "Cet utilisateur ne peut pas recevoir d’avertissement de ta part.",
                    "Permission refusée",
                    "0x203",
                    false
                )
        ...
        try {
            await db?.promise().query(`INSERT INTO sanctions (sanction_id, author_id, user_id, reason, timestamp, type, duration) VALUES (?, ?, ?, ?, ?, ?, ?)`, [sanctionId, interaction.user.id, userTargeted.id, reason, timestamp, "Avertissement", "Aucune"]);
        } catch {
            await interaction.reply(
                errorMessage(
                    bot,
        ...
        const text_dm = new TextDisplayBuilder().setContent(`## Tu as reçu un avertissement\n\n${process.env.RED_HAMMER} **Modérateur :** <@${interaction.user.id}>\n${process.env.RED_STAR} **Raison :** _${reason}_\n\n${process.env.GREEN_PAPILLON} **Référence de la sanction :** ${sanctionId}`);
